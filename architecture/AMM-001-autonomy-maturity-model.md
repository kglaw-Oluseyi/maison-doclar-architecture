...
**Evidence-earned promotion (v1.1, DI-045):** every promotion cites operational evidence, never opinion. Default gate (tunable per capability in its spec, never waivable to zero): a defined volume of successful executions at the current level (default 100), manual-correction rate under a ceiling (default <1%), zero governance violations in the window, decision-trace completeness (every action DecisionRecord-closed), and rollback demonstrated in production conditions. The promotion register entry cites the telemetry.
