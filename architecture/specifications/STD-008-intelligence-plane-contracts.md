...
**A. AdvisoryRequest** (Execution → Intelligence): protocolVersion, requestId (globally unique, preserved end-to-end), topic, scoped context — scope category declared per DI-048 (Event · Enterprise · Operational · Repository · Capability · Session); guest data always Event-scoped (self-contained per DI-019 — no discovery back into the requester, ever), pinned versions (MPS, MKC, supported envelope versions), deadline. **B. AdvisoryEnvelope** (Intelligence → Execution): protocolVersion, requestId (echoed), advisoryId (globally unique), producer, recommendation, confidence (calibrated; **confidence theater is prohibited** — wide intervals, staleness flags, and "prior-only, no observed evidence" provenance are mandatory where true, per APE §2.8), reasoning (host-readable), provenance, mpsVersion/mkcVersion/intelligenceVersion, generatedAt/expiresAt. **C. FactSubmission** (Execution → Intelligence, via a *declared pipeline* only): pipelineId+version, scope-declared facts (guest facts always Event-scoped), MKC sensitivity classes, consent basis where required. **D. LessonSubmission** (STD-007 ED-10 capture → C22): structured observation with venue/vendor/MPS tags. **E. DecisionRecord** (Execution → Decision Intelligence): requestId, advisoryId, disposition (accepted/modified/rejected/absent), actor, reason. **F. PipelineDeclaration** (registration, not runtime): source module, fact schema, sensitivity ceiling, purpose, retention — the MKC gate.
