...
## 2. Invariants
G-1 **Event-scoped absolutely**: every aggregate carries eventId; no query joins across events (fossil: event-scope verification; Negative Knowledge #2). G-2 **Household is the invitation unit; engagement is the attendance unit** (ratified taxonomy). G-3 **Phone is the within-event dedup key**, normalized per the validated normalizer lineage (`phone-normalize.ts` donor); fuzzy name-matching only ever *proposes*, never auto-merges. G-4 **Five-part RSVP model** (DI-076, resolving this section's prior provisional three-state text): response state (`not_yet_invited → invited → responded(accepted|declined|tentative) → confirmed`) · attendance mode (in-person/remote/hybrid-undecided) · allocation state (`offered/waitlisted/released` — the host's capacity decision, never the guest's) · engagement state (`responsive/unresponsive_since(date)/unreachable` — derived from contact timing, never human-toggled) · attendance record (`arrived/no_show/turned_away/arrived_uninvited` — gate-recorded truth, append-only, never overwrites response state). "No response" is never a state (fossil `retire_rsvp_no_response` still honoured: silence is engagement-state territory, not response-state territory). G-5 **RSVP never gates** any downstream system; it informs eligibility only. G-6 **Respondent ≠ attendee**: the person who answers is modelled distinctly from those attending (ratified data-acquisition principle). G-7 **Imports never race allocation** (fossil `guest_list_import_lock`): import lock is a module-level primitive exposed to seating. G-8 **Every guest mutation is revisioned** with actor and source (Nucleus pending-change/revision donor pattern); host/planner-originated changes enter a review state before commit. G-9 **Duplicate handling is a ledger, not an event**: proposals, dismissals, merges, and couple-split reviews are first-class records (Nucleus + Aperture donors). G-10 **Data quality is graded G0–G5** per guest and per event, computed not asserted, and consumable by readiness (the go-live computation donor). G-11 Every record pins the MPS version in force at creation; taxonomy fields (titles, tiers, household types, group types) are MPS references, never local enums (Negative Knowledge #5). G-12 Tenancy is present in the schema from day one (A-006) even while single-tenant in operation.
...
## 6. Open items
O-1 **Closed by DI-076.** Final RSVP flag/mode shape — founder ruling on corpus §6. O-2 MPS v1.0 publication (G-11 dependency). O-3 Relationship-declaration richness (strength scales) — reconcile with STD-001's consumption during seating implementation.
